...
- convert to VM image
- 'common' env: auto created for holding epkg manager files
...
- on `epkg init --store=shared`, auto run `epkg env create common --public`
  (store is shared == common env is public)

- when there are public common env, normal user may still create personal
  common env with explicit command `epkg env create common`

- runtime search path is
  - first try find private common env
  - then try find public common env

Example private common env dir layout:
```
~/.epkg/envs/common/usr/bin/epkg
~/.epkg/envs/common/usr/bin/init -> epkg
~/.epkg/envs/common/usr/bin/x2epkg -> epkg
~/.epkg/envs/common/usr/bin/create-repo -> epkg
~/.epkg/envs/common/usr/bin/elf-loader
~/.epkg/envs/common/opt/epkg-manager -> epkg-master
~/.epkg/envs/common/opt/epkg-master  # general form: epkg-$version, source code in epkg.git
~/.epkg/envs/common/opt/epkg-master/lib/epkg-rc.sh # sourced by .bashrc/.zshrc
```
...
rustc example setup:
```
cp ~/.epkg/envs/common/usr/bin/elf-loader ~/.epkg/envs/main/usr/ebin/rustc
...
options root=/dev/sda1 init=/opt/epkg/envs/root/common/usr/bin/init generation=5
```
