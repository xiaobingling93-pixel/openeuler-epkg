...
| **Windows** | Conda/msys2（原生）+ Linux 发行版（VM，libkrun） | 单二进制（按需拉取 Linux 侧 epkg 等资产，与 macOS 同类） |

### 1.2 各平台目标详解

#### Windows
- **原生支持**：Conda/msys2 包（Windows `.exe` 等），scriptlets 在宿主直接执行
- **VM 支持**：Linux 发行版包（deb/rpm/arch/apk 等）经 `IsolateMode::Vm` + **libkrun** microVM（与 macOS 同一路径；宿主需使用启用了 `libkrun` Cargo feature 的构建，并具备 sandbox-kernel / 依赖，见 `docs/design-notes/sandbox-vmm.md`）。默认是否在 `FEATURES=auto` 下启用见 `bin/make.sh` 的 `should_enable_libkrun`（当前 Windows 为 **x86_64**）
- **可选**：用户仍可在 WSL 内使用 Linux 版 epkg；宿主 Windows 上 libkrun 提供不依赖 WSL 的 Linux 执行环境
...
### 1.3 macOS / Windows VM Sandbox 工作流程
...
| **Linux 宿主 busybox** | `busybox/init.rs`, `busybox/vm_daemon.rs`, `busybox/modprobe.rs` 等（随 applet cfg） | ✅ | ❌ | ❌ |
| **libkrun（VM 后端）** | `libkrun` | ✅ | ✅ | ✅（启用 `libkrun` feature 时） |
...
Windows 移植的目标是支持原生 Conda/msys2 包以及（可选）Linux 发行版包经 libkrun VM，这意味着大量代码应该在 Windows 上直接工作。
...
5. **symlink 透明化**：
   - `lfs::symlink_file_for_virtiofs()` 和 `lfs::symlink_file_for_native()` 在 Windows 上正常工作
   - 调用者根据意图选择 API：virtiofs（VM guest）或 native（host 访问）
   - 底层自动处理 symlink、hardlink、junction 或 LX reparse
...
1. **Junction 需要绝对路径**：
   - 相对路径会在 `lfs::symlink_dir_for_native()` 中被转换为绝对路径
...
- `IsolateMode::Vm`：使用 libkrun；**macOS、Windows** 在构建启用 `libkrun` 特性且环境满足 VMM 依赖时支持
- `IsolateMode::None`：Conda/Homebrew/msys2 使用
  - 对应 `run_options.skip_namespace_isolation` flag
  - 跳过 namespace 隔离，直接在主机环境执行

**VM Sandbox（macOS / Windows 宿主）限制**：
- 无 Linux 式 namespace/bind mount（宿主侧）
...
**难度**: ⭐⭐⭐ (中：原生 Conda/msys2 与 Linux 包 VM 路径并存)

| 差异点 | Linux | Windows | 解决方案 |
|-------|-------|---------|---------|
| symlink | 标准 | 需管理员/开发者模式 | junction (目录) + symlink_file (文件) |
| 权限模型 | POSIX (rwx) | ACL | 原生 Conda/msys2 可弱化 POSIX；**Linux 发行版包** 在 libkrun VM + virtiofs（NTFS EA）中处理 |
| 路径分隔符 | `/` | `\` | 使用 `Path`/`PathBuf` |
| 可执行权限 | `chmod +x` | `.exe` 扩展名 | 检查扩展名 |

**说明**：原生 Conda/msys2 无需宿主 namespace；**Linux 发行版包** 在 Windows 上与 macOS 类似，经 libkrun 隔离执行 scriptlets / `epkg run --isolate=vm`，不依赖 WSL。
...
- macOS / Windows 的 Linux ELF epkg（guest）放在同一相对布局下，便于 `self install` 与 VM rootfs 复用
- Windows 不需要 elf-loader；启用 libkrun VM 时需要 Linux ELF epkg（由 init 下载）
...
### Phase 4: macOS / Windows VM Sandbox 集成（工作量：大）

**目标**：macOS 与 Windows（启用 `libkrun`）支持 Linux 发行版包（scriptlets / `run --isolate=vm` 在 VM 执行）

**前提条件**：
- Phase 1-3 完成（各宿主原生功能就绪）
- libkrun 集成（Linux、macOS、Windows 宿主均可按需启用 `libkrun` feature）
...
| Homebrew 包 | ❌ | ✅ (未来) | ❌ |
| msys2 包 | ❌ | ❌ | ✅ (未来) |
| Debian/RPM/APK | ✅ | ✅ (VM) | ✅ (VM，libkrun) |
| Arch 包 | ✅ | ✅ (VM) | ✅ (VM，libkrun) |
