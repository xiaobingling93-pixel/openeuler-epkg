...
```
/c/iepkg/
├── channel/                         # Final output directory
...
```mermaid
graph TB
    A[External Mirror Sources] --> B[scripts/mirror/fetch_new_mirrors.py]
    B --> C[scripts/mirror/new-mirrors.json]
    ...
    J[channel/manual-mirrors.json] --> K[scripts/mirror/merge_mirrors.py]
    C --> K
    E --> K
    G --> K
    H --> K
...
|------|------|---------|--------|-------|
| `ubuntu.yaml` | `channel/` | Ubuntu distribution directory mappings | YAML | Directory filtering |
| `debian.yaml` | `channel/` | Debian distribution directory mappings | YAML | Directory filtering |
| `fedora.yaml` | `channel/` | Fedora distribution directory mappings | YAML | Directory filtering |
| `archlinux.yaml` | `channel/` | Arch Linux distribution directory mappings | YAML | Directory filtering |
| `alpine.yaml` | `channel/` | Alpine distribution directory mappings | YAML | Directory filtering |
| `*.yaml` | `channel/` | Other distribution configs | YAML | Directory filtering |

### Manual Configuration Files
| File | Path | Purpose | Format | Maintenance |
|------|------|---------|--------|-------------|
| `manual-mirrors.json` | `channel/` | Manually curated mirror configurations | JSON (linewise) | Manual editing |

### Intermediate Processing Files
| File | Path | Purpose | Generated By | Used By |
|------|------|---------|--------------|---------|
| `new-mirrors.json` | `scripts/mirror/` | Newly discovered mirrors | `fetch_new_mirrors.py` | `ls_mirrors.py`, `merge_mirrors.py` |
...
|------|------|---------|--------|---------|
| `mirrors.json` | `channel/` | Final mirror configuration | JSON (compact) | Rust `load_mirrors_for_distro()` |
| `manual-mirrors.json` recommendations | `channel/` | Suggested manual configurations | JSON (linewise) | Rust `load_mirrors_for_distro()` |
...
```mermaid
graph LR
    A[Distribution Websites] --> B[scripts/mirror/fetch_new_mirrors.py]
    B --> C[Parse HTML/Text]
    C --> D[Extract URLs]
    D --> F[scripts/mirror/new-mirrors.json]
...
- **Output**: `scripts/mirror/new-mirrors.json`
- **Cache**: `scripts/mirror/index/*.html`

### Stage 2: Directory Listing
```mermaid
graph LR
    A[scripts/mirror/new-mirrors.json] --> B[scripts/mirror/ls_mirrors.py]
    B --> C[HTTP Requests]
...
**File Operations:**
- **Input**:
  - `scripts/mirror/new-mirrors.json`
  - Previous `scripts/mirror/ls-mirrors.json`
  - `channel/*.yaml` (distribution configs)
...
```mermaid
graph LR
    A[scripts/mirror/new-mirrors.json] --> F[scripts/mirror/merge_mirrors.py]
...
- **Input**:
  ...
  - `scripts/mirror/nocontent-mirrors.txt`
- **Output**:
  - `channel/mirrors.json`

## File Type Color Legend

| Color | File Type | Examples |
|-------|-----------|----------|
| 🔵 Blue | Python Scripts | `fetch_new_mirrors.py`, `ls_mirrors.py`, `probe_dirs.py`, `merge_mirrors.py` |
| 🟠 Orange | JSON Processing Files | `new-mirrors.json`, `ls-mirrors.json`, `probe-mirrors.json` |
...
channel/ubuntu.yaml                       # Ubuntu directory mappings
channel/debian.yaml                       # Debian directory mappings
channel/fedora.yaml                       # Fedora directory mappings
channel/archlinux.yaml                    # Arch Linux directory mappings
channel/alpine.yaml                       # Alpine directory mappings
channel/*.yaml                            # Other distribution configs
```

### Processing File Locations
```shell
# Core processing files
scripts/mirror/new-mirrors.json          # Stage 1 output
...
```shell
# Production files
channel/mirrors.json                     # Final mirror database
```
...
```json
{
  "https://mirror.example.com": {
    "cc": "US",                    // Country code
    "os": ["ubuntu", "debian"],    // Supported distributions
    "dir": ["ubuntu-ports"],       // Distribution directories
    "ls": ["ubuntu", "debian"],    // Listed directories
    "pdir": ["centos-vault"],      // Probed directories
    "root": 1,                     // Top-level mirror flag
...
| `distros` | `os` | Array | Supported operating systems |
| `distro_dirs` | `dir` | Array | Distribution-specific directories |
| `ls` | `ls` | Array | Directories found via listing |
| `probe_dirs` | `pdir` | Array | Directories found via probing |
| `top_level` | `root` | Boolean | Mirror serves from root path |
...
graph TB
    ...
    G[channel/mirrors.json] --> B
    H[channel/manual-mirrors.json] --> C
...
|----------|---------|-----------------|--------|
| `load_primary_mirrors` | Load main mirror data | `channel/mirrors.json` | Raw mirror HashMap |
| `load_and_merge_manual_mirrors` | Merge manual overrides | `channel/manual-mirrors.json` | Updated HashMap |
...
# Input: scripts/mirror/new-mirrors.json + previous scripts/mirror/ls-mirrors.json + channel/*.yaml
# Output: updated scripts/mirror/ls-mirrors.json
...
# Input: scripts/mirror/ls-mirrors.json + channel/*.yaml
...
ls_mirrors.py will print recommendations for `channel/manual-mirrors.json`:
```json
"http://mirror.example.com":{"cc":"US","ls":["ubuntu","debian"]},
```

Copy/paste relevant entries to `channel/manual-mirrors.json`:
```shell
# Edit manual configuration
vim channel/manual-mirrors.json
```
...
# Input: All JSON files + channel/manual-mirrors.json + error files
...
**Example `channel/archlinux.yaml`:**
```yaml
distro_dirs:
  - archlinux
  - archlinux-arm
