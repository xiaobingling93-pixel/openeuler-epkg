...
use crate::utils::force_symlink_file_for_native;
...
/// Get user name by UID by reading /etc/passwd directly.
/// This avoids calling getpwuid() which triggers nscd query on musl when user not found,
/// causing TSI to intercept Unix socket connect and block on macOS.
#[cfg(unix)]
pub fn get_uid_name(uid: u32) -> String {
    if uid == 0 {
        return "root".to_string();
    }
    // Read /etc/passwd directly to avoid nscd query
    if let Ok(content) = std::fs::read_to_string("/etc/passwd") {
        for line in content.lines() {
            let parts: Vec<&str> = line.split(':').collect();
            if parts.len() >= 3 {
                if let Ok(uid_str) = parts[2].parse::<u32>() {
                    if uid_str == uid {
...
/// Get group name by GID by reading /etc/group directly.
/// This avoids calling getgrgid() which triggers nscd query on musl when group not found,
/// causing TSI to intercept Unix socket connect and block on macOS.
#[cfg(unix)]
pub fn get_gid_name(gid: u32) -> String {
    if gid == 0 {
        return "root".to_string();
    }
    // Read /etc/group directly to avoid nscd query
    if let Ok(content) = std::fs::read_to_string("/etc/group") {
        for line in content.lines() {
            let parts: Vec<&str> = line.split(':').collect();
            if parts.len() >= 3 {
                if let Ok(gid_str) = parts[2].parse::<u32>() {
                    if gid_str == gid {
                        return parts[0].to_string();
                    }
                }
...
pub fn create_all_applet_symlinks(env_root: &Path, pkg_format: &PackageFormat) -> Result<()> {
    ...
    for cmd_name in &applet_names {
        ...
        // Use symlink_file_for_native for all targets:
        // - Native Windows: creates proper symlinks with hardlink/copy fallback
        // - Linux/VM targets on Windows: also works correctly for virtiofs access
        // - Unix: same as symlink_file_for_virtiofs (aliased in utils.rs)
        force_symlink_file_for_native(&target, &symlink_path)
    ...
    force_symlink_file_for_native(&bracket_target, &bracket_path)
