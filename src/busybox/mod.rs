...
/// Get user name by UID, avoiding nscd query for root (UID 0).
/// musl's getpwuid falls back to nscd when user not found in /etc/passwd,
/// which can cause TSI to intercept Unix socket connect and block.
/// Returning "root" for UID 0 avoids triggering nscd.
#[cfg(unix)]
...
/// Get group name by GID, avoiding nscd query for root (GID 0).
#[cfg(unix)]
