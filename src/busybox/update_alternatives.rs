...
fn apply_alternative(
    ...
    #[cfg(unix)]
    lfs::symlink_file_for_native(&_relative_target, &alt_name_path)?;
    ...
    #[cfg(unix)]
    {
        ...
        lfs::symlink_file_for_native(&alt_name_relative, &master_link_full)?;
    }
    ...
    for sl in &a.slaves {
        if let Some(spath) = choice.slave_files.get(&sl.name) {
            ...
            #[cfg(unix)]
            {
                ...
                lfs::symlink_file_for_native(&slave_relative, &slave_alt).ok();
            }
            ...
            #[cfg(unix)]
            {
                ...
                lfs::symlink_file_for_native(&slave_alt_relative, &slave_link_full).ok();
            }
        }
    }
    Ok(())
