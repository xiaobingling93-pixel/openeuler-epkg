...
unsafe extern "C" {
    ...
    fn krun_set_idmap(
        ctx_id: u32,
        uid_map_json: *const std::ffi::c_char,
        gid_map_json: *const std::ffi::c_char,
    ) -> i32;
    fn krun_set_kernel(
        ctx_id: u32,
    ...
    let base_cmdline = {
        ...
        format!(
            "reboot=k panic=-1 panic_print=0 nomodule console=ttyS0,115200 {} \
             ...
             disable_kvm_pv=1 clocksource=hyperv_clocksource_tsc_page idle=poll \
             virtio_net.disable=1 virtio_gpu.disable=1 virtio_input.disable=1{} init={}",
...
fn create_and_configure_vm(
    ...
    // Note: HVF VM creation lock is now handled internally by libkrun's setup_vm()
    // in git/libkrun/src/vmm/src/builder.rs. The lock protects hv_vm_create() calls
    // which happen during krun_start_enter() -> build_microvm() -> setup_vm() -> Vm::new().
    // No need for external lock here.
...
pub fn run_command_in_krun(
    ...
    if config.use_vsock {
        ...
        // Reverse mode is only used on non-Linux platforms
        #[cfg(not(target_os = "linux"))]
        ...
        #[cfg(target_os = "linux")]
        if config.use_reverse_vsock {
            log::warn!("libkrun: reverse vsock mode is not supported on Linux");
        }
