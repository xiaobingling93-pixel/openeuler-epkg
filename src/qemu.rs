...
/// Run VM in daemon/keeper mode for QEMU backend.
...
pub fn run_vm_daemon_mode(
    ...
    use crate::vm::{VmConfig, register_vm_session, vm_socket_path_for_env};
    ...
    let run_options = RunOptions {
        ...
        vm_keep_timeout: Some(timeout_secs),
        ..Default::default()
    };
    ...
    // Apply auto-mapping policy for non-root users
    let (auto_uid, auto_gid) = if crate::auto_idmap::should_auto_map(host_uid) {
        crate::auto_idmap::auto_idmap_specs(host_uid, host_gid, None)
    } else {
        (vec![], vec![])
    };

    let translate_uid = crate::auto_idmap::merge_idmap_specs(auto_uid, vec![]);
    let translate_gid = crate::auto_idmap::merge_idmap_specs(auto_gid, vec![]);

    // Setup rootfs mode (virtiofs or 9p)
    let rootfs_mode = setup_rootfs_mode(
        env_root,
        None,
        virtiofsd_bin.as_ref(),
    ...
    // Spawn QEMU (no init_cmd - guest vm_daemon will wait for commands)
    let mut qemu_child = spawn_qemu(
        &kernel,
        &initrd,
        &qemu_bin,
        ...
        cpus as u8,
        memory_mib,
        None,  // init_cmd - daemon mode
        None,  // init_user
    )?;

    log::info!("qemu: VM process spawned (pid={})", qemu_child.id());

    // Get socket path for session registration
    let socket_path = vm_socket_path_for_env(env_root);

    // Wait for guest ready via vsock
    let reuse_session = true;
    let qemu_stderr_path = qemu_log_path.with_extension("stderr.log");
    match vm_client::wait_ready_and_send_command_with_qemu(
        &[],  // no command - just wait for ready
        crate::models::IoMode::Stream,
        10000,  // command port
        None,   // no unix socket - use AF_VSOCK
        reuse_session,
        Some(timeout_secs),
        None,   // no user
        &mut qemu_child,
    ...
    register_vm_session(env_root, env_name, &socket_path, "qemu", &vm_config)?;
