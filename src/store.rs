...
// ============================================================================
// Store Data Integrity: Progressive Disk Space Estimation
// ============================================================================
//
...
// evaluation in risks.rs happens AFTER all packages are unpacked, which is
// too late to prevent partial/corrupted store.
//
// Solution: Progressive estimation per-package granularity:
//
// 1. BEFORE unpack (estimation phase):
//    - Use package.installed_size as baseline (from package metadata in repo)
//    - Add block alignment overhead: file_count_estimate * block_size * 0.75
//    - Add directory overhead: dir_count_estimate * block_size
//    - Add info/ overhead: ~2KB base + 80 bytes per entry
//    - file_count_estimate = installed_size / AVG_FILE_SIZE (43KB)
//    - dir_count_estimate = file_count_estimate / AVG_FILES_PER_DIR (10)
//    - link_count_estimate = file_count_estimate / AVG_FILES_PER_LINK (20)
//    - Use global counters to track cumulative estimated usage
//    - Check: if (current_free - cumulative_estimated) < 100MB, abort early
//
// 2. AFTER unpack (adjustment phase):
//    - Read actual counts from filelist.txt (already created by unpack)
//    - Adjust global counters: subtract estimate, add actual
//    - This makes estimation progressively more accurate as more packages
//      are unpacked (like a Bayesian update)
//
// Key insight: The estimation starts conservative (using averages), then
// becomes more accurate as actual data arrives. Early abort prevents
// wasted work when disk space is critically low.
//
// Constants (derived from real data analysis):
// - AVG_FILE_SIZE: 43KB (from /usr: 23G / 569,537 files)
// - AVG_FILES_PER_DIR: 10 (typical directory contains ~10 files)
// - AVG_FILES_PER_LINK: 20 (symlinks are less common than files)
// - MIN_FREE_SPACE: 100MB safety margin
//

// Average file size derived from real-world data analysis:
// /usr on typical Linux system: 23GB total, 480,995 files, 45,681 dirs, 42,861 symlinks
// => Average file size = 23GB / 480,995 files ≈ 50KB
const AVG_FILE_SIZE: u64 = 50 * 1024; // 50KB

// Typical ratio of files to directories in a package
// 480,995 files / 45,681 dirs ≈ 10.5
const AVG_FILES_PER_DIR: u64 = 10;

// Typical ratio of files to symlinks
// 480,995 files / 42,861 symlinks ≈ 11
const AVG_FILES_PER_LINK: u64 = 11;

// Minimum free space to maintain (safety margin)
const MIN_FREE_SPACE: u64 = 100 * 1024 * 1024; // 100MB

// Global counters for progressive estimation
// These track cumulative disk space usage estimates
#[cfg(unix)]
static ESTIMATED_TOTAL_SIZE: AtomicU64 = AtomicU64::new(0); // Total estimated disk usage
#[cfg(unix)]
static ACTUAL_FILE_COUNT: AtomicU64 = AtomicU64::new(0);    // Actual files unpacked
#[cfg(unix)]
static ACTUAL_DIR_COUNT: AtomicU64 = AtomicU64::new(0);     // Actual dirs unpacked
#[cfg(unix)]
static ACTUAL_LINK_COUNT: AtomicU64 = AtomicU64::new(0);    // Actual symlinks unpacked
#[cfg(unix)]
static ACTUAL_PACKAGE_COUNT: AtomicU64 = AtomicU64::new(0); // Packages unpacked

/// Reset global estimation counters (called before starting a new installation batch)
#[cfg(unix)]
pub fn reset_estimation_counters() {
    ESTIMATED_TOTAL_SIZE.store(0, Ordering::Relaxed);
    ACTUAL_FILE_COUNT.store(0, Ordering::Relaxed);
    ACTUAL_DIR_COUNT.store(0, Ordering::Relaxed);
    ACTUAL_LINK_COUNT.store(0, Ordering::Relaxed);
    ACTUAL_PACKAGE_COUNT.store(0, Ordering::Relaxed);
}

#[cfg(not(unix))]
pub fn reset_estimation_counters() {}

/// Get current estimation counters (useful for debugging/testing)
#[cfg(unix)]
#[allow(dead_code)]
pub fn get_estimation_counters() -> (u64, u64, u64, u64, u64) {
    (
        ESTIMATED_TOTAL_SIZE.load(Ordering::Relaxed),
        ACTUAL_FILE_COUNT.load(Ordering::Relaxed),
        ACTUAL_DIR_COUNT.load(Ordering::Relaxed),
        ACTUAL_LINK_COUNT.load(Ordering::Relaxed),
        ACTUAL_PACKAGE_COUNT.load(Ordering::Relaxed),
    )
}
...
/// Uses package.installed_size as baseline, adds filesystem overhead
/// This is a conservative estimate based on average file sizes
#[cfg(unix)]
pub fn estimate_package_disk_space(pkgkey: &str) -> Result<u64> {
    // Get package info from cache (has installed_size from repo metadata)
    let package = crate::package_cache::load_package_info(pkgkey)
        .map_err(|e| eyre!("Failed to load package info for {}: {}", pkgkey, e))?;

    // installed_size is the declared installed size (in bytes)
    let installed_size = package.installed_size as u64;

    // Estimate file count based on average file size
    // If installed_size is 0, assume at least 10 files (small packages)
    ...
    // Estimate directory count (typically 1 dir per 10 files)
    let dir_count_estimate = file_count_estimate / AVG_FILES_PER_DIR + 1;

    // Estimate symlink count (typically 1 symlink per 20 files)
    let link_count_estimate = file_count_estimate / AVG_FILES_PER_LINK;

    // Get filesystem block size for overhead calculation
    ...
    // Calculate disk space similar to risks.rs validate_before_linking():
    // 1. Block alignment overhead for files: file_count * block_size * 0.75
    let file_block_overhead = file_count_estimate * block_size * 3 / 4;

    // 2. Directory overhead: dir_count * block_size
    let dir_overhead = dir_count_estimate * block_size;

    // 3. Info/ directory overhead: ~2KB base + 80 bytes per entry
    ...
    log::trace!(
        "estimate_package_disk_space: pkgkey={}, installed_size={}, file_est={}, dir_est={}, link_est={}, \
         block_size={}, file_overhead={}, dir_overhead={}, info_overhead={}, total_estimate={}",
        pkgkey, crate::utils::format_size(installed_size), file_count_estimate, dir_count_estimate, link_count_estimate,
        block_size, crate::utils::format_size(file_block_overhead), crate::utils::format_size(dir_overhead),
        crate::utils::format_size(info_overhead), crate::utils::format_size(total_estimate)
    );
...
/// Check if there's enough disk space BEFORE unpacking a package
/// Uses progressive estimation: cumulative estimated + this package's estimate
/// Returns error if (free_space - cumulative_estimated) < MIN_FREE_SPACE
#[cfg(unix)]
pub fn check_store_space_before_unpack(pkgkey: &str) -> Result<()> {
    ...
    // Estimate this package's disk usage
    let pkg_estimate = estimate_package_disk_space(pkgkey)?;

    // Get cumulative estimated usage
    let cumulative_estimate = ESTIMATED_TOTAL_SIZE.load(Ordering::Relaxed);

    // Predict final free space after all planned packages
    let predicted_final_free = store_fs_info.free_space.saturating_sub(cumulative_estimate + pkg_estimate);

    log::trace!(
        "check_store_space_before_unpack: pkgkey={}, free={}, cumulative={}, this_estimate={}, predicted_final_free={}",
        pkgkey, crate::utils::format_size(store_fs_info.free_space),
        crate::utils::format_size(cumulative_estimate), crate::utils::format_size(pkg_estimate),
        crate::utils::format_size(predicted_final_free)
    );

    if predicted_final_free < MIN_FREE_SPACE {
        return Err(eyre!(
            "Insufficient disk space predicted: {} available after unpacking {}. \
             Current free: {}, cumulative estimated: {}, this package: {}. \
             Packages unpacked: {}, files: {}, dirs: {}, links: {}. \
             Abort early to prevent partial/corrupted store.",
            crate::utils::format_size(predicted_final_free),
            pkgkey,
            crate::utils::format_size(store_fs_info.free_space),
            crate::utils::format_size(cumulative_estimate),
    ...
    // Add this package's estimate to cumulative (will be adjusted after actual unpack)
    ESTIMATED_TOTAL_SIZE.fetch_add(pkg_estimate, Ordering::Relaxed);
...
/// Adjust counters AFTER unpack using actual filelist.txt data
/// This makes estimation progressively more accurate
#[cfg(unix)]
fn adjust_counters_after_unpack(store_tmp_dir: &Path, estimated_size: u64) -> Result<()> {
    // Read actual entry counts from filelist.txt
    let filelist_path = store_tmp_dir.join("info").join("filelist.txt");

    if !lfs::exists_on_host(&filelist_path) {
        // filelist.txt not yet created (shouldn't happen, but handle gracefully)
        log::warn!("filelist.txt not found at {}, skipping counter adjustment", filelist_path.display());
        return Ok(());
    }

    // Parse filelist.txt to get actual counts
    let content = std::fs::read_to_string(&filelist_path)
        .wrap_err_with(|| format!("Failed to read filelist.txt: {}", filelist_path.display()))?;
    ...
    // Calculate actual disk usage using the same formula as risks.rs
    let store_root = dirs().epkg_store.clone();
    let store_fs_info = crate::risks::get_filesystem_info(&store_root);
    let block_size = store_fs_info.block_size.max(4096);

    // Block alignment overhead for actual files
    let file_block_overhead = actual_file_count * block_size * 3 / 4;
    let dir_overhead = actual_dir_count * block_size;

    // Info overhead (this package's filelist.txt already accounts for entries)
    const INFO_BASE_OVERHEAD: u64 = 2 * 1024;
    const FILELIST_BYTES_PER_ENTRY: u64 = 80;
    ...
    // Get installed_size from package.txt (already written by unpack)
    let package_txt_path = store_tmp_dir.join("info").join("package.txt");
    let installed_size = if lfs::exists_on_host(&package_txt_path) {
    ...
    // Adjust ESTIMATED_TOTAL_SIZE: subtract estimate, add actual
    // This makes subsequent predictions more accurate
    let prev_estimate = ESTIMATED_TOTAL_SIZE.fetch_sub(estimated_size, Ordering::Relaxed);
    ESTIMATED_TOTAL_SIZE.fetch_add(actual_size, Ordering::Relaxed);

    // Update actual counters
    ACTUAL_FILE_COUNT.fetch_add(actual_file_count, Ordering::Relaxed);
    ACTUAL_DIR_COUNT.fetch_add(actual_dir_count, Ordering::Relaxed);
    ACTUAL_LINK_COUNT.fetch_add(actual_link_count, Ordering::Relaxed);
    ACTUAL_PACKAGE_COUNT.fetch_add(1, Ordering::Relaxed);

    log::trace!(
        "adjust_counters_after_unpack: file={}, dir={}, link={}, installed_size={}, actual_size={}, estimated_size={}, \
         cumulative_estimate: {} -> {} -> {}",
        actual_file_count, actual_dir_count, actual_link_count, crate::utils::format_size(installed_size),
        crate::utils::format_size(actual_size), crate::utils::format_size(estimated_size),
        crate::utils::format_size(prev_estimate),
        crate::utils::format_size(prev_estimate - estimated_size),
        crate::utils::format_size(ESTIMATED_TOTAL_SIZE.load(Ordering::Relaxed))
    );
...
#[cfg(not(unix))]
fn adjust_counters_after_unpack(_store_tmp_dir: &Path, _estimated_size: u64) -> Result<()> {
    Ok(())
}
...
pub fn unpack_mv_package_with_format(
    ...
    // Phase 1: Estimate disk space BEFORE unpack
    // Check if there's enough space for this package based on installed_size
    // and cumulative estimation from previous packages
    #[cfg(unix)]
    let estimated_size: u64 = if let Some(pk) = pkgkey {
        // Check and add estimate to cumulative counter
        check_store_space_before_unpack(pk)?;
        // Get the estimated size we just added
        estimate_package_disk_space(pk)?
    } else {
        0 // Skip estimation for manual unpack without pkgkey
    };
    #[cfg(not(unix))]
    ...
    // Phase 2: Unpack the package (with optional format hint)
    ...
    // Phase 3: Adjust counters AFTER unpack using actual filelist.txt data
    // This makes estimation progressively more accurate for subsequent packages
    #[cfg(unix)]
    if pkgkey.is_some() && estimated_size > 0 {
        adjust_counters_after_unpack(&store_tmp_dir, estimated_size)?;
    }
...
fn collect_store_pkglines() -> Result<(std::collections::HashMap<String, Vec<String>>, std::collections::HashMap<String, Vec<String>>)> {
    ...
    if let Ok(entries) = fs::read_dir(&store_dir) {
        for entry in entries.flatten() {
            ...
            if package_path.is_dir() {
                ...
                // Check if fs directory exists
                // (for Move link type, files are moved to env but fs/ still exists)
                if !lfs::exists_on_host(&fs_dir) {
                ...
                // Skip consumed packages (Move link type - files moved to another env)
                let consumed_marker = package_path.join("info/consumed.json");
                if consumed_marker.exists() {
                    log::debug!("Skipping consumed package {}", package_path.display());
                    skipped_missing_fs += 1;
                    continue;
                }
    ...
    log::debug!("collect_store_pkglines: found {} pkglines ({} skipped), organized into {} pkgkey entries, {} pkgname entries",
