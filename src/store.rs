...
/// Compares multiple fields: pkgname, version, arch, source, sha256sum, sha1sum
fn packages_match(repodata_pkg: &crate::models::Package, store_pkg: &crate::models::Package) -> bool {
    // Must match on basic identity fields
    if repodata_pkg.size != store_pkg.size
        || repodata_pkg.installed_size != store_pkg.installed_size {
        return false;
    }
    ...
    if let (Some(repodata_sha1), Some(store_sha1)) = (&repodata_pkg.sha1sum, &store_pkg.sha1sum) {
        if repodata_sha1 == store_sha1 {
            return true; // Strong match via SHA1
        } else {
            return false;
        }
    }

    // If checksums aren't available, still consider it a match
    // if basic fields match (pkgname, version, arch already checked above)
    true
}
