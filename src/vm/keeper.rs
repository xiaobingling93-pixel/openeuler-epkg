...
//!
//! The keeper process:
//! 1. Creates and configures the VM
//! 2. Registers the VM session
//! 3. Runs VM (blocking)
//! 4. Cleans up on exit
//!
//! For libkrun: runs VM in a thread within the keeper process.
//! For QEMU: spawns QEMU as a subprocess and waits for it.
...
/// Delegates to qemu.rs for actual implementation.
#[cfg(target_os = "linux")]
fn run_qemu_keeper(env_root: &Path, env_name: &str, config: &VmConfig) -> Result<()> {
    crate::qemu::run_qemu_daemon_mode(
        env_root,
        env_name,
        config.timeout,
        config.cpus,
...
fn run_qemu_keeper(_env_root: &Path, _env_name: &str, _config: &VmConfig) -> Result<()> {
    Err(eyre!("QEMU backend only supported on Linux"))
}