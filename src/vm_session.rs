...
//!
//! Session file location: `{epkg_run}/vm-sessions/{env_hash}.json`
//! Socket path pattern: `{epkg_run}/vsock-{env_hash}.sock`
...
pub fn discover_vm_session(env_root: &Path) -> Result<Option<VmSessionInfo>> {
    ...
    if !is_process_alive(info.pid) {
        log::debug!("vm_session: session process {} is dead, cleaning up", info.pid);
...
pub fn register_vm_session(env_root: &Path, socket_path: &Path) -> Result<()> {
        ...
        for entry in entries.flatten() {
            ...
            if let Ok(content) = std::fs::read_to_string(&path) {
                if let Ok(info) = serde_json::from_str::<VmSessionInfo>(&content) {
                    ...
                    if !is_process_alive(info.pid) {
                        log::debug!("vm_session: cleaning up stale session for PID {}", info.pid);
