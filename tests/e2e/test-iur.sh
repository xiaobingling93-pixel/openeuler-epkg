...
while IFS=: read os packages; do
    ...
    if "$SCRIPT_DIR/test-one.sh" $DEBUG_FLAG "cases/install-remove-upgrade.sh" "$os" $packages; then
...
if "$SCRIPT_DIR/test-one.sh" $DEBUG_FLAG "cases/install-remove-upgrade.sh" "all-os" curl wget vim; then
